...
class _AiPackagesScreenState extends State<AiPackagesScreen> {
        ...
        child: TweenAnimationBuilder<double>(
          ...
          child: Material(
            ...
            child: Container(
              ...
              child: Row(
                children: [
                  Icon(isError ? Icons.error_outline_rounded : Icons.check_circle_outline_rounded, color: Colors.white, size: 21),
                  const SizedBox(width: 10),
                  Expanded(child: Text(message, style: const TextStyle(color: Colors.white, fontFamily: 'Siemreap', fontWeight: FontWeight.w600, fontSize: 13))),
                ],
              ),
            ),
          ),
  ...
  Future<void> _saveQrToGallery() async {
    try {
      ...
      if (!await Gal.hasAccess()) {
        throw Exception(_t('សូមអនុញ្ញាតឱ្យ App រក្សាទុករូបភាពក្នុង Gallery', 'Please allow the app to save images to Gallery.'));
      }
      final byteData = await rootBundle.load('assets/aba_qr.png');
      final tempDir = await getTemporaryDirectory();
      final file = File('${tempDir.path}/sesan_aba_qr.png');
      await file.writeAsBytes(byteData.buffer.asUint8List(byteData.offsetInBytes, byteData.lengthInBytes));
      await Gal.putImage(file.path);
      if (!mounted) return;
      _showTopMessage(_t('បានរក្សាទុក QR ក្នុង Gallery រួចរាល់!', 'QR saved to Gallery successfully!'));
    } catch (e) {
      if (!mounted) return;
      _showTopMessage(_t('មិនអាចរក្សាទុក QR បាន: $e', 'Unable to save QR: $e'), isError: true);
    }
  }
                ...
                children: [
                  ...
                  GestureDetector(
                    ...
                    child: Container(
                      ...
                      child: Image.asset('assets/aba_qr.png', height: 175, fit: BoxFit.contain,
                        errorBuilder: (_, __, ___) => const SizedBox(height: 175, child: Center(child: Icon(Icons.qr_code_2_rounded, size: 120, color: Colors.black54)))),
                    ),
                  ),
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (active && _expiresAtMs > 0)
            Text('${_t('Credit នៅសល់', 'Credits left')}: $_remainingCredits/$_creditLimit · ${_t('ផុតកំណត់', 'Expires')}: ${_date(_expiresAtMs)}', style: const TextStyle(fontFamily: 'Siemreap', color: Colors.white70, fontSize: 10)),
          if (_requestReceiptUrl.isNotEmpty)
            TextButton.icon(onPressed: _showSubmittedReceipt, style: TextButton.styleFrom(foregroundColor: Colors.white, padding: EdgeInsets.zero, visualDensity: VisualDensity.compact), icon: const Icon(Icons.receipt_long_outlined, size: 16), label: Text(_t('មើលវិក្កយបត្រដែលបានផ្ញើ', 'View submitted receipt'), style: const TextStyle(fontFamily: 'Siemreap', fontSize: 10.5))),
        ])),
  ...
  Widget _freeCard() => _planCardShell(
    ...
    child: ListTile(
      ...
      subtitle: Text(_t('10 Credits ក្នុងមួយថ្ងៃ', '10 credits per day'), style: const TextStyle(fontFamily: 'Siemreap', fontSize: 11)),
        ...
        child: Row(children: [
          ...
          if (active && !_renewalIsOpen)
            ...
            FilledButton(
              ...
              child: Text(active ? _t('ជាវបន្ត', 'Renew') : _t('ជាវ', 'Buy'), style: const TextStyle(fontFamily: 'Siemreap', fontSize: 11)),
            ),
        ]),
      ),
    );
