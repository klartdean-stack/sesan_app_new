...
/// V51 Product List shell for iOS. It keeps the existing ProductGridView/card
/// implementation while adding Android V51 AI image/voice search and Cambodia
/// province/district/commune filtering UI.
...
class _ProductListV51ScreenState extends State<ProductListV51Screen> {
  ...
  bool get _hasLocation =>
      ...
      (_commune?.isNotEmpty ?? false);

  String get _locationLabel => <String>[
        if (_province?.isNotEmpty ?? false) _province!,
        if (_district?.isNotEmpty ?? false) _district!,
        if (_commune?.isNotEmpty ?? false) _commune!,
      ].join(' › ');
  ...
  Future<void> _applyAi(String value) async {
    ...
    _searchController.selection = TextSelection.collapsed(offset: primary.length);
    ...
    await showModalBottomSheet<void>(
      ...
      builder: (sheetContext) => StatefulBuilder(
        builder: (context, setSheetState) {
          ...
          Widget field(String label, String? value, List<String> items,
              ValueChanged<String?> changed, bool enabled) {
            return Padding(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            icon: Icon(_hasLocation ? Icons.filter_alt : Icons.filter_alt_outlined),
          ),
        ],
                ...
                Container(
                  ...
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      suffixIconConstraints: const BoxConstraints.tightFor(width: 96),
                      suffixIcon: Row(
                        children: [
                          SizedBox(
                            width: 32,
                            child: IconButton(
                              ...
                              icon: const Icon(Icons.image_search, color: Colors.purple, size: 21),
                            ),
                          ),
                          SizedBox(
                            width: 32,
                            child: IconButton(
                              ...
                              icon: Icon(
                                ai?.recording == true ? Icons.stop_circle : Icons.mic,
                                color: ai?.recording == true ? Colors.red : Colors.green,
                                size: 21,
                              ),
                            ),
                          ),
                          SizedBox(
                            ...
                            child: IconButton(
                              ...
                              icon: const Icon(Icons.qr_code_scanner, color: Colors.blue, size: 21),
                            ),
                          ),
                        ],
                      ),
