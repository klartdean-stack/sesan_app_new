...
class _EditProductScreenState extends State<EditProductScreen> {
  ...
  Future<void> _updateProduct() async {
    ...
    try{final urls=List<String>.from(_existingImageUrls);for(final f in _imageFiles){final r=FirebaseStorage.instance.ref('products/images/${DateTime.now().microsecondsSinceEpoch}.jpg');await r.putFile(f);urls.add(await r.getDownloadURL());}
      ...
      await FirebaseFirestore.instance.collection('products').doc(widget.productId).update({'product_name':_nameController.text.trim(),'price':_priceController.text.trim().replaceAll(',', ''),'description':_descController.text.trim(),'track_stock':stock.isNotEmpty,if(stock.isNotEmpty)'stock_quantity':n,if(stock.isNotEmpty)'stock_unit':_stockUnitController.text.trim(),if(stock.isNotEmpty)'is_available':(n??0)>0,'phone1':_phoneController.text.trim(),'location':_locationController.text.trim(),'lat':selectedLat,'lng':selectedLng,'image_urls':urls,'updatedAt':FieldValue.serverTimestamp(),'seller_id':widget.productData['seller_id']});
