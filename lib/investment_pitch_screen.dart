...
class _InvestmentPitchScreenState extends State<InvestmentPitchScreen> {
  ...
  Future<void> _saveToFirestore(Map<String, dynamic> data) async {
    await FirebaseFirestore.instance.collection('investment_requests').add(data);
  }
  ...
  Future<Map<String, List<int>>> _fetchMonthlyData() async {
    ...
    try {
      ...
      for (var doc in results[0].docs) {
        ...
        if (date != null && date.year == currentYear) uMonthly[date.month - 1]++;
      }
      for (var doc in results[1].docs) {
        var data = doc.data() as Map<String, dynamic>;
        DateTime? date = data['created_at']?.toDate();
        if (date != null && date.year == currentYear) pMonthly[date.month - 1]++;
      }
      for (var doc in results[2].docs) {
        var data = doc.data() as Map<String, dynamic>;
        DateTime? date = data['created_at']?.toDate();
        if (date != null && date.year == currentYear) oMonthly[date.month - 1]++;
      }
                ...
                return PopupMenuButton<String>(
                  ...
                  itemBuilder: (context) => [
                    const PopupMenuItem(
                      ...
                      child: Row(
                        children: [
                          Icon(Icons.inventory_2, color: Colors.redAccent, size: 20),
                          SizedBox(width: 10),
                    ...
                    const PopupMenuItem(
                      ...
                      child: Row(
                        children: [
                          Icon(Icons.swap_horiz, color: Colors.redAccent, size: 20),
                          SizedBox(width: 10),
                          Text("សំណើផ្ទេរ"),
                        ],
                      ),
          ...
          builder: (context, snapshot) {
            ...
            return Column(
              children: [
                ...
                if (snapshot.hasData &&
                    (snapshot.data!.data() as Map<String, dynamic>)['price_change'] != null)
                  Text(
                    "${(snapshot.data!.data() as Map<String, dynamic>)['price_change'] > 0 ? '+' : ''}${(snapshot.data!.data() as Map<String, dynamic>)['price_change']}% សប្ដាហ៍នេះ",
                    style: TextStyle(
                      color: ((snapshot.data!.data() as Map<String, dynamic>)['price_change'] ?? 0) >= 0
                          ? Colors.greenAccent
                          : Colors.redAccent,
                      fontSize: 12,
                    ),
  ...
  Widget _buildStockStats() {
    return StreamBuilder<DocumentSnapshot>(
      stream: FirebaseFirestore.instance.collection('app_equity_stats').doc('current').snapshots(),
      builder: (context, snapshot) {
            ...
            children: [
              Row(
                ...
                children: [
                  ...
                  _statColumn("ហ៊ុនដាក់លក់", "$listedShares", Colors.orangeAccent),
  ...
  Widget _buildTrustIndicators() {
    return Row(
      children: [
        Expanded(child: _trustCard(Icons.verified_user, "ចុះបញ្ជីច្បាប់", "...", Colors.blueAccent)),
        const SizedBox(width: 10),
        Expanded(child: _trustCard(Icons.security, "ប្រព័ន្ធសុវត្ថិភាព", "SSL + Escrow", Colors.greenAccent)),
        const SizedBox(width: 10),
        Expanded(child: _trustCard(Icons.local_shipping, "ប្រព័ន្ធដឹកជញ្ជូន", "គ្របដណ្តប់ទូទាំងប្រទេស", Colors.orangeAccent)),
      ],
    );
  }
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Text(subtitle, textAlign: TextAlign.center, style: TextStyle(color: color, fontSize: 10)),
        ],
      ),
    );
  }
      ...
      child: Row(
        children: [
          Container(
            ...
            child: const Icon(Icons.warning_amber, color: Colors.orangeAccent, size: 20),
          ),
        ...
        children: [
          ...
          IconButton(
            ...
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(builder: (_) => const InvestmentTermsScreen()),
              );
            },
          ),
        ],
  ...
  Widget _stepIndicator(int step, int currentStep, IconData icon) {
    ...
    return Container(
      ...
      height: 40,
      decoration: BoxDecoration(
        color: isActive ? const Color(0xFF3B5BFF) : Colors.white.withOpacity(0.1),
        shape: BoxShape.circle,
  ...
  Widget _stepLine(int step, int currentStep) {
    return Expanded(
      child: Container(
        height: 2,
        color: step < currentStep ? const Color(0xFF3B5BFF) : Colors.white.withOpacity(0.1),
      ),
    );
  }
      ...
      case 0:
        return Column(
          children: [
            ...
            _buildDarkInput("អាសយដ្ឋាន *", addressController, Icons.location_on),
          ],
        );
      case 1:
        return StreamBuilder<DocumentSnapshot>(
            ...
            return Column(
              children: [
                TextFormField(
                  ...
                  decoration: _darkInputDecoration('ចំនួនហ៊ុនដែលស្នើទិញ *', Icons.pie_chart),
                ...
                Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      const Text(
                        'តម្លៃក្នុង ១ ហ៊ុន:',
                        style: TextStyle(color: Colors.white54, fontFamily: 'Siemreap', fontSize: 13),
                      ),
                      Text(
                ...
                Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      const Text(
                        'សរុបត្រូវបង់:',
                        style: TextStyle(color: Colors.white70, fontFamily: 'Siemreap'),
                      ),
                      Text(
                ...
                DropdownButtonFormField<String>(
                  ...
                  decoration: _darkInputDecoration('ជ្រើសរើសធនាគារ *', Icons.account_balance),
                ...
                const SizedBox(height: 15),
                _buildDarkInput('លេខគណនីរបស់អ្នក *', bankAccController, Icons.credit_card),
              ],
            );
          },
        );
            ...
            Container(
              ...
              child: Column(
                children: [
                  ...
                  const Text(
                    "សឺមីកូដបង់ប្រាក់",
                    style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontFamily: 'Siemreap'),
                  ),
                  const SizedBox(height: 10),
                  CachedNetworkImage(
                    imageUrl:
                  ...
                  ElevatedButton.icon(
                    ...
                    label: const Text("បើក App ABA", style: TextStyle(color: Colors.white)),
                    style: ElevatedButton.styleFrom(
                    ...
                    child: Container(
                      ...
                      child: _receiptImage == null
                          ? Column(
                              mainAxisAlignment: MainAxisAlignment.center,
                              children: [
                                Icon(Icons.add_a_photo, color: Colors.white.withOpacity(0.3), size: 40),
                                const SizedBox(height: 8),
                                Text(
  ...
  Future<void> _submitInvestment(
    ...
    try {
      ...
      final formData = {
        ...
        'bank_name': 'ABA',
        'total_price': int.parse(shareController.text) * currentPriceFromFirebase,
        'receipt_url': url,
        'status': 'pending',
