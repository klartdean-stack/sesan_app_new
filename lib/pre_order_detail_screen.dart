...
class _PreOrderDetailScreenState extends State<PreOrderDetailScreen> {
  ...
  String _maskedPhone(dynamic value) {
    ...
    final prefixLength = phone.startsWith('+')
        ? (phone.length < 4 ? phone.length : 4)
        : (phone.length < 3 ? phone.length : 3);
  ...
  void _revealPhoneTemporarily() {
    _phoneHideTimer?.cancel();
    _showPhoneNotifier.value = true;
    _phoneHideTimer = Timer(const Duration(seconds: 5), () {
      if (mounted) _showPhoneNotifier.value = false;
    });
  }

  void _hidePhone() {
    _phoneHideTimer?.cancel();
    _showPhoneNotifier.value = false;
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(
          _t('ព័ត៌មានលម្អិត', 'Pre-order details'),
          ...
          IconButton(
            ...
            onPressed: () {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text(
                    _t('បានចម្លង Link រូបភាព', 'Image link copied'),
                  ),
                ),
              );
            },
            ...
            onPressed: () {
              ...
              Share.share(
                _t(
                  ...
                  'Pre-order: ${widget.data['product_name'] ?? ''}\nPrice: ${widget.data['price'] ?? '0'} KHR\nView image: $firstImg',
                ),
              );
            },
          ),
  ...
  Widget _buildPosterCard() {
    ...
    return GestureDetector(
      onTap: () {
        final userId = widget.data['owner_id'];
  ...
  Widget _buildBottomBar(bool isOwner) => Container(
  ...
  Widget _buildImageGallery(List<dynamic> images) => Stack(
    children: [
      SizedBox(
        height: 320,
  ...
  void _goToChat() {
    if (currentUid == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            _t('សូមចូលប្រើប្រាស់កម្មវិធីជាមុនសិន', 'Please log in first'),
          ),
        ),
      );
      return;
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (_) => ChatScreen(
          ...
          productName: widget.data['product_name'] ?? _t('ផលិតផល', 'Product'),
  ...
  Widget _buildTitlePriceSection() => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      Row(
          ...
          children: [
            ...
            Text(
              _t(
                ...
                'Posted: ${_formatDate(widget.data['created_at'])}',
              ),
              style: const TextStyle(
                fontSize: 12,
                color: Colors.grey,
  ...
  Widget _buildProductDetails() => Column(
    children: [
      _row(
        Icons.event_available,
        _t('ថ្ងៃប្រមូលផល', 'Harvest date'),
      ...
      _row(
        ...
        _t('ទីតាំង', 'Location'),
        widget.data['location'] ?? _t('មិនមានទីតាំង', 'No location'),
      ),
      const Divider(),
  ...
  Widget _buildPhoneRow() {
    final fullPhone = (widget.data['phone'] ?? '').toString().trim().replaceAll(
      RegExp(r'\s+'),
      '',
    );
    return ValueListenableBuilder<bool>(
      valueListenable: _showPhoneNotifier,
      builder: (context, showFullPhone, _) {
        final displayedPhone = fullPhone.isEmpty
            ? _t('មិនមានលេខទូរស័ព្ទ', 'No phone number')
            : (showFullPhone ? fullPhone : _maskedPhone(fullPhone));
        return Padding(
          padding: const EdgeInsets.symmetric(vertical: 10),
          child: Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Icon(Icons.phone_android, color: Colors.green, size: 22),
              ...
              if (fullPhone.isNotEmpty)
                IconButton(
                  onPressed: showFullPhone
                      ? _hidePhone
                      : _revealPhoneTemporarily,
                  icon: Icon(
                    showFullPhone ? Icons.visibility_off : Icons.visibility,
                    color: Colors.green.shade700,
                    size: 20,
                  ),
                  tooltip: showFullPhone
                      ? _t('លាក់លេខ', 'Hide number')
                      : _t('បង្ហាញលេខ 5 វិនាទី', 'Show number for 5 seconds'),
  ...
  Widget _buildDescriptionRow() {
    ...
    final displayText = description.isEmpty
        ? _t('មិនមានការពិពណ៌នា', 'No description')
        : description;
    ...
    return Padding(
      ...
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Icon(Icons.description_outlined, color: Colors.green, size: 22),
          const SizedBox(width: 15),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
                ...
                if (shouldShowToggle)
                  Align(
                    ...
                    child: TextButton.icon(
                      ...
                      label: Text(
                        _isDescriptionExpanded
                            ? (isEnglish ? 'See less' : 'បង្រួម')
                            : (isEnglish ? 'See more' : 'មើលបន្ថែម'),
                        style: const TextStyle(
                          fontFamily: 'Siemreap',
                          fontWeight: FontWeight.bold,
                        ),
  ...
  String _formatDate(dynamic date) {
    if (date == null) return _t('មិនទាន់កំណត់', 'Not set');
