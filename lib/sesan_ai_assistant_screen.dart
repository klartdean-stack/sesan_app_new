...
class _SesanAiAssistantScreenState extends State<SesanAiAssistantScreen> {
  ...
  Future<HttpsCallable?> _authenticatedAssistantCallable() async {
    ...
    if (user == null) {
      try {
        ...
        debugPrint('Sesan AI auth restore delayed: $e');
  ...
  String _sessionExpiredMessage() => _t(
        'សម័យចូលប្រើបានផុតកំណត់។ សូមចូលគណនីម្តងទៀត។',
  ...
  Future<void> _loadCredits() async {
    try {
      final callable = await _authenticatedAssistantCallable();
      ...
      final remaining = int.tryParse(
            (entitlement['remainingCredits'] ?? creditLimit).toString(),
          ) ??
          creditLimit;
      if (!mounted) return;
      setState(() => _remainingCredits = remaining);
    } catch (error) {
      debugPrint('Could not load Sesan AI credits: $error');
    }
  }
      ...
      if (callable == null) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(_sessionExpiredMessage())),
          );
        }
        return;
      }
  ...
  Future<void> _revealAssistantAnswer({
    required String answer,
    required String messageId,
    required List<_AssistantSource> sources,
  }) async {
    if (!mounted) return;
    ...
    setState(() {
      ...
      _messages.add(const _AssistantMessage(text: '', isUser: false));
    });
    _scrollToBottom();
    ...
    for (var i = 0; i < pieces.length; i++) {
      ...
      final shouldPaint = i == pieces.length - 1 || i % 2 == 1;
      if (shouldPaint && mounted && messageIndex < _messages.length) {
        setState(() {
          _messages[messageIndex] = _AssistantMessage(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(
          _t('Sesan AI ជំនួយការ', 'Sesan AI Assistant'),
          maxLines: 1,
  ...
  Widget _buildMessages() {
    return ListView.builder(
      ...
      itemCount: _messages.length + (_sending && !_revealingAnswer ? 1 : 0),
            ...
            child: Container(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Text(
                    _t('Sesan AI កំពុងរៀបចំចម្លើយ...', 'Sesan AI is preparing an answer...'),
                    style: const TextStyle(
                      fontSize: 12,
                      fontFamily: 'Siemreap',
