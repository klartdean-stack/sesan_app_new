...
class _ChatListScreenState extends State<ChatListScreen> {
        ...
        body: Center(
          child: Column(
            ...
            children: [
              ...
              Text('chat_login_required'.tr,
                  style: const TextStyle(fontSize: 18, color: Colors.orange)),
              const SizedBox(height: 24),
              ElevatedButton(
                onPressed: () => Navigator.pop(context),
                child: Text('chat_go_back'.tr),
              ),
            ],
          ),
        ),
                ...
                children: [
                    ...
                    child: Container(
                      ...
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          hintText: 'chat_search_sesan_hint'.tr,
                  ...
                  GestureDetector(
                    onTap: () => Navigator.push(
                      context,
                      MaterialPageRoute(builder: (_) => const FarmToolsPage()),
                    ),
                    child: Container(
  ...
  Widget _buildChatTile(Map<String, dynamic> chat, String otherUserId) {
    return StreamBuilder<DocumentSnapshot>(
      ...
      builder: (context, userSnapshot) {
        ...
        String userName = userData?['name'] ?? 'chat_user'.tr;
        ...
        return ListTile(
          ...
          subtitle: Row(
            children: [
              ...
              Expanded(
                child: Text(
                  isOnline ? 'chat_online'.tr : _formatLastSeen(userData?['lastSeen']),
                  maxLines: 1,
                  overflow: TextOverflow.ellipsis,
                  style: TextStyle(
                    fontSize: 11,
  ...
  String _formatLastSeen(dynamic timestamp) {
    if (timestamp == null) return 'chat_inactive'.tr;
    final time = (timestamp as Timestamp).toDate();
    final diff = DateTime.now().difference(time);
    if (diff.inMinutes < 1) return 'chat_active_now'.tr;
    if (diff.inMinutes < 60) {
      return 'chat_active_minutes'.trParams({'count': '${diff.inMinutes}'});
    }
    if (diff.inHours < 24) {
      return 'chat_active_hours'.trParams({'count': '${diff.inHours}'});
    }
    return 'chat_active_days'.trParams({'count': '${diff.inDays}'});
  }
  ...
  Future<void> _searchById(String id) async {
    ...
    try {
      ...
      if (result.docs.isEmpty) {
        setState(() {
          _searchError = 'chat_id_not_found'.trParams({'id': id});
      ...
      setState(() {
        _searchError = '❌ ${Localizations.localeOf(context).languageCode == 'en' ? 'Error' : 'មានបញ្ហា'}: $e';
        _isSearchLoading = false;
      });
    }
  }
  ...
  Widget _buildEmptyState() => Center(
        child: Column(
          ...
          children: [
            ...
            Text('chat_start_conversation'.tr,
  ...
  String _getSubtitleText(Map<String, dynamic> chat) {
    if (chat['type'] == 'image') return 'chat_image'.tr;
    if (chat['type'] == 'video') return 'chat_video'.tr;
    if (chat['type'] == 'audio') return 'chat_audio'.tr;
    return chat['message'] ?? "";
  }
